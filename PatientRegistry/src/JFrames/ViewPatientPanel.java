...
class ViewPatientPanel extends JPanel {
	...
	/**
	 * The ViewPatientPanel() is the constructor which inherits froma a
	 * JPanel.When called, it constructs a JPanel that includes a number of
	 * components such as textfields and buttons that allow the user to view and
	 * edit a new patient's information.
	 */
	public ViewPatientPanel() {
		...
		PatientInfoPanel.setBackground(new Color(153, 255, 204));
		...
		dateChooserDOB.setBackground(new Color(153, 255, 204));
		...
		PatientInfoPanel.add(panel_2, "cell 2 5 1 2,grow");
		...
		PatientInfoPanel.add(scrollPaneIllnesses, "flowx,cell 2 12,grow");
		...
		panelAppointments.setBackground(new Color(153, 255, 204));
		...
		dateChooserApp.setBackground(new Color(153, 255, 204));

		dateChooserApp.setBounds(68, 0, 135, 28);
		panelAppointments.add(dateChooserApp);

		JButton btnSearchIllness = new JButton("Search Illness");
		btnSearchIllness.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {

				Desktop desktop = java.awt.Desktop.getDesktop();
				// declare an array of strings to allow search for up to 5
				// simultaneous illnesses
				String[] ill = new String[5];
				String illnesses = MainJFrame.VPP.textAreaIllnesses.getText();
				// pass the five first illnesses into to the string array
				ill = illnesses.split("\\W", 5); // http://stackoverflow.com/questions/5993779/java-use-split-with-multiple-delimiters
				// loop though the array opening a tab for each of the illnesses
				for (int i = 0; i < ill.length; i++) {
					if (!ill[i].isEmpty()) {
						URI oURL;
						try {
							oURL = new URI("https://www.google.co.uk/#q=" + ill[i] + "illness");
							desktop.browse(oURL);
		...
		PatientInfoPanel.add(btnSearchIllness, "cell 2 12,alignx trailing");

		JPanel panelEmergencyContact = new JPanel();
		panelEmergencyContact.setBackground(new Color(153, 255, 204));
		...
		panelImages.setBackground(new Color(153, 255, 204));
		...
		btnDeleteImage.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				// Ask the user if he/she wants to delete the image assigned to
				// the patient
				int dialogueResult = JOptionPane.showConfirmDialog(btnDeleteImage,
						"Are you sure you want to delete this image?");
				if (dialogueResult == JOptionPane.YES_OPTION) {
					// the patient's profile photo is saved and named using the
					// patient ID
					int row = MainJFrame.HP.table.getSelectedRow();
					String patientPhoto = MainJFrame.HP.database.get(row)[0].toString() + ".png";
					// instantiate the photo using the patientID and then delete
					// the file
					File profilePic = new File(patientPhoto);
					profilePic.delete();
					// reset the lbl showing a black panel
					lblPatientPhoto.setIcon(null);
				}
			}
		});
		btnDeleteImage.setBounds(226, 249, 117, 29);
		panelImages.add(btnDeleteImage);

		JTabbedPane tabbedPane = new JTabbedPane(JTabbedPane.TOP);
		tabbedPane.setBounds(6, 6, 337, 232);
		panelImages.add(tabbedPane);
		panelPatientPhoto.setBackground(Color.WHITE);
		tabbedPane.add("Patient Image", panelPatientPhoto);
		panelPatientPhoto.setLayout(null);
		lblPatientPhoto.setHorizontalAlignment(SwingConstants.CENTER);

		lblPatientPhoto.setBounds(6, 6, 304, 174);
		panelPatientPhoto.add(lblPatientPhoto);
		panelIllnessPhotos.setBackground(new Color(153, 255, 204));
		...
		btnImage1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				ImageViewer IV = new ImageViewer(MainJFrame.VPP.lblGeneratedPatientID);
				IV.setVisible(true);
		...
		btnImage1.setBounds(95, 16, 117, 29);
		...
		btnImage2.setBounds(95, 57, 117, 29);
		...
		btnImage3.setBounds(95, 98, 117, 29);
		...
		btnImage4.setBounds(95, 139, 117, 29);
		panelIllnessPhotos.add(btnImage4);
		JButton btnUploadImage = new JButton("Update/Upload Image");
		btnUploadImage.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				int dialogueResult = JOptionPane.showConfirmDialog(btnUploadImage,
						"Are you sure you want to save changes to the patient's \n" + "profile picture?");
				if (dialogueResult == JOptionPane.YES_OPTION) {
					// save image and update the panel

					Methods.saveImageVPP();
					Methods.updateProfilePicVPP();
				}

			}
		});
		btnUploadImage.setBounds(6, 249, 171, 29);
		...
		btnSaveChanges.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (textFieldName.getText().isEmpty() || textFieldSurname.getText().isEmpty()
					...
					// file.
					int dialogueResult = JOptionPane.showConfirmDialog(btnSaveChanges,
							"Are you sure you want to save the \n changes you made?");
					if (dialogueResult == JOptionPane.YES_OPTION) {

						// SAVE THE DATA TO AN ARRAY OF OBJECTS
						Object[] patientData = new String[20];

						patientData[0] = (lblGeneratedPatientID.getText()); // 0
						patientData[1] = textFieldName.getText().replace(",", " "); // 1
						patientData[2] = textFieldSurname.getText().replace(",", " ");// 2
						Format formatter = new SimpleDateFormat("dd-MM-yyyy");
						String DOB = formatter.format(dateChooserDOB.getDate());
						patientData[3] = DOB;// 3
						patientData[4] = genders[comboBoxGender.getSelectedIndex()]; // 4
						patientData[5] = textFieldPatientMobile.getText().replace(",", " ");// 5
						patientData[6] = textFieldPatientPhone.getText().replace(",", " ");// 6
						patientData[7] = textFieldEmail.getText().replace(",", " ");// 7
						patientData[8] = textFieldAddressLine1.getText().replace(",", " ");// 8
						patientData[9] = textFieldAddressLine2.getText().replace(",", " ");// 9
						patientData[10] = textFieldPostcode.getText().replace(",", " ");// 10
						patientData[11] = textFieldEmergencyContact.getText().replace(",", " ");// 11
						patientData[12] = textFieldrelationshipToPatient.getText().replace(",", " ");// 12
						patientData[13] = textFieldContactNumber.getText().replace(",", " ");// 13

						patientData[14] = textAreaBilling.getText().replace("\n", " ").replace(",", " ");// 14

						patientData[15] = textAreaIllnesses.getText().replace("\n", " ").replace(",", " ");// 15

						patientData[16] = textAreaComments.getText().replace("\n", " ").replace(",", " ");// 16
						Format formatter2 = new SimpleDateFormat("dd-MM-yyyy");
						String LatestAppointment = formatter2.format(dateChooserApp.getDate());
						patientData[17] = textFieldHour.getText().replace(",", " ");// 17
						patientData[18] = textFieldMinutes.getText().replace(",", " ");// 18
						patientData[19] = LatestAppointment;// 19

						// FIND THE OLD DATA AND REMOVE IT
						String id = lblGeneratedPatientID.getText();
						...
						// ADD THE EDITTED DATA TO THE DATABASE
						MainJFrame.HP.database.add(patientData);
		...
		btnSaveChanges.setBounds(847, 519, 117, 29);
		add(btnSaveChanges);
		...
		btnBackToHome.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					// RESET THE TABLE
					BufferedReader buff = new BufferedReader(new FileReader("patients.csv"));
				...
				Methods.emptyFieldsVPP();
				Methods.whiteBackgroundVPP();
			}
		});
		...
		btnDelete.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				// ensure the doctor wants to delete the patient records
				int dialogueResult = JOptionPane.showConfirmDialog(btnDelete,
						"Are you sure you want to delete this patients record?\n This action is irreversible!");
				if (dialogueResult == JOptionPane.YES_OPTION) {
					// first delete the patient profile photo named
					// "patientID.png"
					Methods.deleteProfilePhoto();
					// delete the the four scans of the patient
					for (int j = 1; j <= 4; j++) {
						Methods.deleteCurrentImage(j);
					}
					// delete the patients data from the database
					String id = lblGeneratedPatientID.getText();
					for (int i = 0; i < MainJFrame.HP.database.size(); i++) {
						if (id.equals(MainJFrame.HP.database.get(i)[0].toString())) {
							MainJFrame.HP.database.remove(i);
		...
		btnDelete.setBounds(733, 519, 117, 29);
		add(btnDelete);

		JLabel lblVieweditPatient = new JLabel("View/Edit Patient");
		lblVieweditPatient.setForeground(new Color(0, 51, 255));
		lblVieweditPatient.setFont(new Font("Lucida Grande", Font.PLAIN, 33));
		lblVieweditPatient.setBounds(682, 24, 282, 29);
		add(lblVieweditPatient);
